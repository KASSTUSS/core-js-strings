...
function removeFirstOccurrences(str, value) {
  ...
  if (index === -1) {
    return str; // value not found in str
  }
...
function findLongestWord(sentence) {
  ...
  let longestWord = '';
  const words = sentence.split(/\s+/); // Splitting the sentence into words
...
function reverseWords(str) {
  ...
  return str
    ...
    .map((word) => word.split('').reverse().join('')) // Reversing each word
    .join(' '); // Joining the reversed words back into a string
}
...
function invertCase(str) {
  ...
  return str
    .split('') // Splitting the string into characters
    .map((char) => {
      // Inverting the case of each character
      if (char === char.toUpperCase()) {
        return char.toLowerCase();
      }
      return char.toUpperCase();
    })
    .join(''); // Joining the characters back into a string
}
...
function extractNameFromTemplate(value) {
  ...
  if (value.startsWith('Hello, ') && value.endsWith('!')) {
    // Remove 'Hello, ' from the start and '!' from the end
...
function extractEmails(str) {
  ...
  // Split the string using semicolon as the delimiter
  return str.split(';');
}
